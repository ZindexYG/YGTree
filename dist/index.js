...
class YGTree {
    ...
    ContentsFilter(path) {
        ...
        const fileSort = folder.concat(documented);
        return fileSort;
    }
    // 写
    WiteFileConsole(files, level = 1, isLast = false) {
        const arr = files.map((item, index) => {
            ...
            if (item.contents &&
                ...
                level !== 0 &&
                level !== this.level) {
                const symbol = item.contents.length === 1 && index === files.length - 1
                    ? `${levelStr}└── `
                    : `${levelStr}├── `;
                const isLast = item.contents.length === 1 && index === files.length - 1;
                return `${symbol}${item.name}\n${this.WiteFileConsole(item.contents, level + 1, isLast)}`;
            }
    ...
    // 过滤 .gitignore
    FilterIgnoresFile(path) {
        var _a;
        const files = fs__default["default"].readdirSync(path, {
            withFileTypes: true,
        });
        const gitignore = (_a = files.find(item => item.name === '.gitignore')) === null || _a === void 0 ? void 0 : _a.name;
        if (!gitignore) {
            return [];
        }
        const gitignoreContext = fs__default["default"].readFileSync(gitignore, 'utf-8');
...
const getPackageJson = () => {
    const packagePath = path__default["default"].join(path__default["default"].resolve(__dirname, '..'), 'package.json');
    let packageJson;
    // 读取文件
    try {
        packageJson = fs__default["default"].readFileSync(packagePath, 'utf-8');
    }
    catch (err) {
...
// level
const LevelInvalidArgumentError = (value) => {
...
// pattern
const PattenInvalidArgumentError = (value) => {
    try {
        fs__default["default"].readdirSync(value, { withFileTypes: true });
        return value;
    }
    catch (error) {
...
// 选项定义
program
    .name(getPackageJson().name)
    .description(getPackageJson().description)
    .version(getPackageJson().version)
    .option('-A, --all', '打印所有目录')
    .option('-L, --level <levelNum>', '仅下降级到 level 目录深处', LevelInvalidArgumentError, '-1')
    .option('-P, --pattern <path>', '指定目录', PattenInvalidArgumentError, null)
